...
library BokkyPooBahsRedBlackTreeLibrary {
    ...
    function getNode(Tree storage self, address key) internal view returns (address _returnKey, address _parent, address _left, address _right, bool _red, uint _value) {
        require(exists(self, key));
        return(key, self.nodes[key].parent, self.nodes[key].left, self.nodes[key].right, self.nodes[key].red, self.nodes[key].value);
    }

    function insert(Tree storage self, address key, uint value) internal {
        require(key != EMPTY);
        require(!exists(self, key));
        ...
        if (cursor == EMPTY) {
            self.root = key;
        } else if (self.nodes[key].value < self.nodes[cursor].value) {
    ...
    function treeMinimum(Tree storage self, address key) public view returns (address) {
